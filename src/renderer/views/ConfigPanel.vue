...
================================================================================
  ...
  2026-07-20 清理了 ~300 行死代码（.roster-* / .cfg-* / .switch / .card 等）。
    ...
    7. 底部操作区        — .config-footer / .action-btn / .footer-tools-bar
...
<template>
  <div class="config-layout" :style="rootStyle">
    ...
    <div class="config-right">
      <TabLogs ref="tabLogsRef" :appInfo="appInfo" />

      <!-- 底部操作区 -->
      <div class="config-footer">
        <div class="footer-left">
          <div class="footer-tools-bar">
            <button class="footer-tool-btn" @click="handleReloadLogs" title="重新从磁盘读取最新日志">
              <i class="fa-solid fa-rotate-right"></i> 重载日志
            </button>
            <button class="footer-tool-btn" @click="openIssues" title="在 GitHub 反馈 Issue 缺陷或建议">
              <i class="fa-regular fa-circle-question"></i> 反馈问题
            </button>
            <button class="footer-tool-btn" @click="openGithub" title="访问 GitHub 开源项目仓库">
...
/* 底部工具条方法 */
const tabLogsRef = ref(null)

function handleReloadLogs() {
  if (tabLogsRef.value?.reload) {
    tabLogsRef.value.reload()
  }
}

function openIssues() {
  const url = 'https://github.com/Yun-Hydrogen/blue-random/issues'
  if (window.configPanelApi?.openUrl) {
    window.configPanelApi.openUrl(url)
  } else {
...
function openGithub() {
  const url = 'https://github.com/Yun-Hydrogen/blue-random'
  if (window.configPanelApi?.openUrl) {
    window.configPanelApi.openUrl(url)
  } else {
    window.open(url, '_blank')
