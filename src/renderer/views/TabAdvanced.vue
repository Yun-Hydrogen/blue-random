 ...
 *      → 通过 NativeBridge 调用 Windows 原生 OpenFileDialog，筛选 .exe 文件
 ...
 *
 * 注意：
 *   - 这是一个 async 函数，因为文件对话框调用是异步的（需要等待用户操作对话框）
 *   - 使用扩展运算符 ...props.admin 创建新对象，
 *     确保 Vue 能检测到数据变化（响应式要求）
 *   - 如果用户在对话框中点击"取消"，不做任何操作
 *   - window.configPanelApi 由 nativeBridgeAdapter 注入，
 *     如果 API 不可用（如非 WebView2 宿主环境），调用会报错
 */
